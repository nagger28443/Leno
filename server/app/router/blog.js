...
const getPrivateBlogs = async ({ ctx, countSql, listSql, commonCond }) => {
...
const serachBlog = async ({ ctx, search, countSql, listSql, commonCond, hasPrivate }) => {
...
const getBlogByCategory = async ({ ctx, category, countSql, listSql, commonCond, hasPrivate }) => {
...
const getBlogByLabels = async ({ ctx, labels, countSql, listSql, commonCond, hasPrivate }) => {
...
const getBlogByArchive = async ({ ctx, archive, countSql, listSql, commonCond, hasPrivate }) => {
  let whereSql = ''
  if (/^\d{4}$/.test(archive)) {
...
router.get('/list', async ctx => {
  ...
  if (isPrivate) {
    await getPrivateBlogs({ ctx, countSql, listSql, commonCond })
    return
  }

  // 搜索博客
  if (search) {
    await serachBlog({ ctx, search, countSql, listSql, commonCond, hasPrivate })
    return
  }

  // 获取某类目下博客列表
  if (category) {
    await getBlogByCategory({ ctx, category, countSql, listSql, commonCond, hasPrivate })
    return
  }
  // 获取具有某标签的博客列表
  if (labels) {
    await getBlogByLabels({ ctx, labels, countSql, listSql, commonCond, hasPrivate })
    return
  }

  // 按归档日期获取博客列表
  if (archive) {
    await getBlogByArchive({ ctx, archive, countSql, listSql, commonCond, hasPrivate })
    return
  }
  // 获取首页列表,待文章内容
  if (hasDetail) {
...
router.post('/', async ctx => {
  const { title, category, labels = '', content, isPrivate = 0 } = ctx.request.body
