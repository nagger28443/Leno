...
router.get('/list', async ctx => {
  ...
  if (search) {
    const sql = `SELECT title,DATE_FORMAT(date,'%Y-%m-%d') as date,category,labels,visit_cnt as visitCount ${
      hasDetail ? `,content` : ``
    } FROM blogs where title LIKE '%${search}%' || category LIKE '%${search}%' || labels LIKE '%${search}%'`
